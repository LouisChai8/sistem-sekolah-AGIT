        ...
        <div class="mt-3 border border-[#E5E3DB] bg-white">
            <div class="flex items-start justify-between border-b border-[#E5E3DB] bg-[#FCFBF8] px-8 py-6">
                <div>
                    ...
                    <p class="mt-1 font-mono text-xs text-slate-500">ID: {{ $class['id'] }}</p>
                </div>
            ...
            <div class="flex justify-end gap-4 border-t border-[#E5E3DB] px-8 py-5">
                ...
                <form action="{{ route('classes.destroy', ['id' => $class['id']]) }}" method="POST" onsubmit="return confirm('Hapus data kelas ini dari buku induk?')">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
