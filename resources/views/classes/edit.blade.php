...
@section('content')
    ...
    <div class="mb-8 border-b border-[#E5E3DB] pb-5">
        ...
        <p class="mt-1 text-sm text-slate-500">Memperbarui catatan atas kelas <span class="font-medium text-[#16213A]">{{ $class['name'] }}</span>.</p>
    </div>
        ...
        <div>
            ...
            <select id="major_id" name="major_id"
                ...
                @foreach ($majors as $major)
                    <option value="{{ $major['id'] }}" {{ (isset($class['major_id']) && $class['major_id'] == $major['id']) || (isset($class['major']) && $class['major'] == $major['code']) ? 'selected' : '' }}>
                        {{ $major['name'] }}
                    </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select id="teacher_id" name="teacher_id"
                ...
                @foreach ($teachers as $teacher)
                    <option value="{{ $teacher['id'] }}" {{ (isset($class['teacher_id']) && $class['teacher_id'] == $teacher['id']) || (isset($class['homeroom_teacher']) && $class['homeroom_teacher'] == $teacher['name']) ? 'selected' : '' }}>
                        {{ $teacher['name'] }}
                    </option>
                @endforeach
            </select>
